...
%
% Note:
% For Adaptive CUSUM^2, the straight path is not displayed.
...
for j = 1:nMethods
    ...
    % Default: display all paths
    plotIdx = 1:nPaths;

    % For Adaptive CUSUM^2, do not display the straight path
    if strcmp(methodName, 'Adaptive CUSUM^2')
        plotIdx = 2:nPaths;
    end

    y = means(plotIdx, j);
    e = errors(plotIdx, j);
    currentLabels = pathLabels(plotIdx);
    currentCounts = counts(plotIdx, j);
