...
filename = 'run22_140_spoofed.csv';
...
xline(140, 'c--', 'LineWidth', 1.5);

% Mark first intersection between s_max and gyro_s
plot(x_int_gyro, y_int_gyro, 'mx', ...
    'MarkerSize', 12, ...
    'LineWidth', 1.5);

% Mark first intersection between s_max and threshold
plot(x_int_thresh, y_int_thresh, 'kx', ...
    'MarkerSize', 12, ...
    'LineWidth', 1.5);
...
xlabel('Distance [m]');
ylabel('CUSUM value');
title('Adaptive CUSUM analysis of spoofed at 140m turns flight');

legend('S_G', ...
       'max(S^+, S^-)', ...
       'standard CUSUM threshold', ...
       'spoofing start (140m)', ...
       'Adaptive CUSUM^2 detection', ...
       'CUSUM detection', ...
       'Location', 'best');