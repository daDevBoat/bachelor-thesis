...
% Y axis: sliding-window sum of abs(column 1 - column 2)
...
for fileNum = 1:25
    ...
    % Sliding-window sums over 25 consecutive values
    x = conv(xDiff, ones(windowSize, 1), "valid");
    y = conv(yDiff, ones(windowSize, 1), "valid");
...
ylabel("25-sample window sum of diff GPS and OF");
title("Sliding-window summed differences");
