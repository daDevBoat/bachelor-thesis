    ...
    while i < len(df):
        if df.loc[i]["of_distance"] == 0 and df.loc[i]["gps_distance"] == 0 and df.loc[i]["gyro_magnitude"] == 0 and df.loc[i]["prev_gyro_magnitude"] == 0:
            ...
            if TEST_PRINTING and run > 1:
                ...
                if spoofed:
                    print(f"Spoofing detected during this run, {updates_to_detection} updates after spoofing started.\n")
...
main("flight_logs_old/turn_control_log.csv", "flight_logs_old/turn_spoofing100_log.csv")
